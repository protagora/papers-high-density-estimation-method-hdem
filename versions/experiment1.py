...
def compute_errors(true_counts, estimated_densities, bin_width, total_samples, data, dist_name, num_bins):
    estimated_counts = np.array(estimated_densities) * bin_width * total_samples
    true_prob_dist = true_counts / total_samples
    estimated_prob_dist = estimated_counts / np.sum(estimated_counts)  # Normalize the estimated counts
    ...
    kl_divergence = entropy(true_prob_dist, estimated_prob_dist) if np.all(estimated_prob_dist) else np.nan
...
for dist_name in distributions:
    for sample_size in sample_sizes:
        ...
        for num_bins in bin_counts:
            ...
            errors_hdem = compute_errors(true_counts, estimated_densities_hdem, bin_width, sample_size, data, dist_name, num_bins)

            # Regular Histogram
            hist_counts, _ = compute_regular_histogram(data, num_bins)
            errors_hist = compute_errors(true_counts, hist_counts, bin_width, sample_size, data, dist_name, num_bins)
